...
> **文档版本**: v2.1
> **创建日期**: 2026-07-21
> **最后更新**: 2026-07-26（v2.1: 修复 Stage 6/A 课程数不一致）
...
| L05 | 字符串与文件处理 | 8 | 文件操作、with 上下文、类基础、继承 |
| L06 | 面向对象进阶 | 6 | 属性访问、@property、类变量 vs 实例变量、MRO、mixin |
| L07 | 魔术方法 | 10 | `__init__`、`__str__`、`__iter__`、`__enter__`、`__getitem__`、描述符协议 |
| L08 | 异常处理 | 8 | try/except、raise、自定义异常、traceback |
| L09 | Python 基础实战 | 6 | 项目结构、argparse、logging、综合应用 |
...
| L12 | Python 高级特性 | 7 | 闭包、nonlocal、@contextmanager、异步上下文 |
| L13 | 描述符与属性 | 6 | `__get__`/`__set__`/`__delete__`、property 底层 |
| L14 | 并发编程入门 | 7 | asyncio、async/await、Task、Future、EventLoop |
| L15 | 函数式编程 | 8 | map/filter/reduce、functools、partial、缓存 |
| L16 | 正则表达式 | 6 | re 模块、match/search、findall、group、贪婪/非贪婪 |
...
| 课程 | 标题 | 知识点数 | 核心知识点 |
|------|------|----------|------------|
| L17 | Pytest 完整实战 | 8 | fixture、Mock、parametrize、conftest、覆盖率 |
| L18 | 现代化工具链 | 6 | uv、ruff、mypy、pre-commit、CI 配置 |
| L19 | 异步编程核心 | 10 | asyncio.run()、TaskGroup、gather、shield、取消 |
| L20 | 装饰器深度探索 | 9 | 参数装饰器、类装饰器、functools.wraps、装饰器工厂 |
| L21 | Python 新特性 | 6 | PEP 695、match-case、异常组、类型参数 |
| L22 | 高阶流控与异步协同 | 6 | Semaphore、Event、Condition、Barrie |
| L23 | 极限抽象与性能优化 | 6 | `__slots__`、猴子补丁、元类、abc |
| L24 | 线程与并发 | 7 | Thread、Lock、RLock、Queue、线程池 |
| L25 | 工程化综合项目 | 5 | TDD 实战、CI/CD 集成、生产级代码规范 |
...
|------|--------|----------|----------|--------|
| Stage 0 | 9 | 73 | 8.1 | 100% |
| Stage 1 | 7 | 50 | 7.1 | 100% |
