...
| `02_modules.py` | 模块与包：`__all__`、公开/私有 API、字典配置、入口点 | 可运行文件内自测；相关模块 API 可参考 `solutions/validators.py` 和 `solutions/__init__.py` |

## 学习路径

1. **01_functions.py** — 先关注函数参数、返回值、递归终止条件和异常处理。
2. **02_modules.py** — 再关注模块 API 设计、`__all__`、公开/私有函数和入口点模式。
