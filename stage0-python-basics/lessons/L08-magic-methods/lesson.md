...
5. ✅ 理解可调用对象：`__call__`
6. ✅ 掌握迭代器协议 `__iter__`（容器协议进阶内容，深入理解可迭代对象与迭代器）
...
> **📌 提示**：`__iter__` 是容器协议中的重要内容，本节将详细讲解迭代器协议，帮助你深入理解可迭代对象与迭代器的关系。
...
| **容器协议** | `__len__`, `__contains__`, `__iter__` | 集合操作、迭代 |
...
- [ ] 实现容器协议 `__len__`、`__contains__`、`__iter__`
- [ ] 实现 `__call__` 使对象可调用
