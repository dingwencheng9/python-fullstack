...
1. **任务分解**：ReAct、CoT、ToT 分解策略
2. **Plan-and-Execute**：规划阶段与执行阶段分离
3. **LLM Compiler**：并行执行子任务，优化调度
4. **Graph of Thoughts**：图结构思维，多路径探索
5. **自我反思**：ReWOO、Reflexion 自我纠正
6. **树搜索**：BFS/DFS/MCTS，探索最优路径
7. **约束验证**：规则校验、可行性检查
8. **动态规划**：子问题复用，缓存中间结果
