...
1. **评估维度**：正确性、效率、鲁棒性、可解释性
2. **Ground Truth**：人工标注、数据集构建、质量保证
3. **自动化评估**：LLM-as-Judge、规则校验、指标计算
4. **轨迹回放**：Trace 分析、步骤验证、问题定位
5. **幻觉检测**：事实核查、一致性检验、置信度评估
6. **性能指标**：成功率、Token 消耗、延迟、召回率
7. **A/B 测试**：实验设计、流量分配、统计显著性
8. **持续监控**：实时告警、趋势分析、回归检测

### 关键要点

- ✅ 评估需要多维度指标
- ✅ LLM-as-Judge 适合开放式任务
- ✅ Trace 回放是调试关键
- ✅ 自动化评估提升迭代效率
- ✅ A/B 测试验证改进效果
...
- 💡 使用 `LangSmith` 收集 Trace
- 💡 使用 `deepeval` 框架自动化评估
- 💡 使用 `Giskard` 检测模型偏见
- 💡 使用 `promptfoo` 对比 Prompt
- 💡 使用 `置信区间` 报告不确定性
- 💡 使用 `回归测试` 防止功能退化
