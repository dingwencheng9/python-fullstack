...
```mermaid
flowchart TB
    subgraph Libraries["可视化库"]
        A[Matplotlib<br/>底层控制] --> B[Seaborn<br/>统计图表]
        B --> C[Plotly<br/>交互图表]
        C --> D[Altair<br/>声明式]
    end

    subgraph ChartTypes["图表类型"]
        E[折线图<br/>时间序列] --> F[柱状图<br/>分类对比]
        G[散点图<br/>相关性] --> H[热力图<br/>矩阵]
        I[饼图<br/>占比] --> J[箱线图<br/>分布]
    end

    subgraph Process["可视化流程"]
        K[数据准备<br/>清洗/聚合] --> L[选择图表<br/>数据特点]
        L --> M[绑定数据<br/>x/y轴]
        M --> N[样式定制<br/>颜色/标签]
        N --> O[导出保存<br/>PNG/SVG/HTML]
    end

    subgraph BestPractices["最佳实践"]
        P[避免3D效果<br/>2D更清晰] --> Q[颜色对比度<br/>色盲友好]
        R[标注关键点<br/>注释数据] --> S[比例一致<br/>纵横比合理]
    end

    style Libraries fill:#e3f2fd
    style ChartTypes fill:#c8e6c9
...
> 💡 **精简提示**：以下内容与 L47 Pandas 课程有重叠，如果你已完成 L47，可以快速浏览或跳过本章节。
