"""练习 2：Token Bucket 速率限制器

目标：实现一个生产级的 Token Bucket 限流器
...
要求：
1. 实现 TokenBucket 类
2. 支持获取 token（阻塞等待或立即返回）
3. 支持突发流量（可以一次性获取多个 token）
4. 线程安全（支持多协程并发使用）

提示：
- Token Bucket 算法：token 以固定速率补充，容量有上限
- 初始时可以有一些突发容量
- 使用 asyncio.Condition 或 asyncio.Lock 实现
"""
...
class TokenBucket:
    ...
    async def acquire(self, tokens: int = 1, blocking: bool = True) -> bool:
        ...
        """
        # TODO: 实现 Token Bucket 获取逻辑

    async def try_acquire(self, tokens: int = 1) -> bool:
        """非阻塞获取 token（快捷方法）"""
        # TODO: 调用 acquire 并设置 blocking=False

    def available_tokens(self) -> float:
        """查看当前可用 token 数量"""
        # TODO: 返回当前可用的 token 数
