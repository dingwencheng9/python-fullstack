...
- ✅ 使用原始字符串 `r"..."` 避免转义问题
- ✅ 多次使用同一模式时用 `re.compile()` 预编译
- ✅ 优先使用 `re.fullmatch()` 进行完全匹配验证
- ✅ 环视断言不消耗字符，用于精确匹配
- ✅ 避免过度复杂的正则，必要时拆分为多个简单正则
...
1. **正则 vs 字符串方法**：为什么 `"foo".replace("o", "a")` 比 `re.sub(r"o", "a", "foo")` 更高效？什么情况下正则表达式是必要的？

2. **贪婪的代价**：正则表达式的 `.*` 是贪婪的，会尽可能多地匹配字符。思考一下，如何用非贪婪匹配 `.*?` 来正确解析 HTML 标签内容？

3. **环视的巧妙应用**：环视断言可以做到一些看似"不可能"的任务。比如，"找出所有不在引号内的单词"，或者"匹配后面跟着特定模式的数字"。思考一下你工作中有什么场景可以用环视解决？
...
- [Regular Expressions 101](https://www.debuggex.com/)
