...
**工程化的核心要素**:
1. **代码组织** - 清晰的目录结构，模块职责单一
2. **自动化测试** - 覆盖核心功能，快速发现问题
3. **工具链集成** - lint、type check、formatting 自动化
4. **文档完善** - README、API 文档、注释
5. **CI/CD** - 自动化构建、测试、部署
...
| 方面 | 随意写法 | 工程化写法 |
|------|----------|------------|
| 函数命名 | `fn()` | `calculate_total()` |
| 错误处理 | `pass` | `raise ValueError()` |
| 类型注解 | 无 | 完整的类型标注 |
| 测试 | 无 | 单元测试 + 集成测试 |
| 文档 | 无 | docstring + README |
| 提交 | 直接提交 | 分支 + PR + review |
...
```python
# ❌ 错误：为了"完美"而过度设计
class BaseAbstractFactoryBuilder:
    def build(self) -> "BaseAbstractFactoryBuilder":
        ...
...
- [L27: HTTP 协议与抓包](../../../stage3-web-basics/lessons/L27-http/) — 互联网通信基础
