"""
L23 测试文件: 极限抽象与性能优化（lesson 概念测试）

本文件验证 **lesson.md 教学概念**本身：
- ``__slots__`` 内存节省的可观测性
- 属性访问性能、继承链、GC 影响等概念演示

与 ``test_l28_complete.py`` 的关系：
- 本文件聚焦于"概念是否成立"——独立于 solutions 文件存在
- ``test_l28_complete.py`` 聚焦于 ``solutions/01_*.py`` / ``02_*.py`` 等
  具体参考答案的 API 与行为，是答案校验
- 两份测试维护成本可控（核心断言不重叠），不要轻率合并
