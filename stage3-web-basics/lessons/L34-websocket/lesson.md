...
```mermaid
flowchart LR
    subgraph Protocol["协议对比"]
        A[SSE 单向] --> B[服务器推送]
        B --> C[HTTP/HTTPS]
        D[WebSocket 双向] --> E[全双工通信]
        E --> F[wss:// 加密]
    end

    subgraph Handshake["WebSocket 握手"]
        G[HTTP GET<br/>Upgrade: websocket] --> H[101 Switching<br/>Protocols]
        H --> I[TCP 长连接]
    end
    ...
    subgraph UseCase["适用场景"]
        J[聊天应用] --> K[✅ WebSocket]
        J --> L[实时协作]
        M[AI 流式对话] --> N[✅ SSE<br/>L32]
        O[实时游戏] --> P[✅ WebSocket]
    end

    subgraph FastAPI["FastAPI WebSocket"]
        Q[@app.websocket] --> R[WebSocket<br/>连接管理]
        R --> S[send_json<br/>receive_json]
        S --> T[connection_manager<br/>广播]
    end
